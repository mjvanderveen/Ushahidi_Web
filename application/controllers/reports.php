...
class Reports_Controller extends Main_Controller {
	...
	public function index()
	{
		...
		if($pagination->total_items == 1) {
			$report_string = Kohana::lang('report.pagination.report_singular');
		} else {
			$report_string = Kohana::lang('report.pagination.report_plural');
		}
		if ($pagination->total_items > 0) {
			...
			if($total_pages > 1) { // If we want to show pagination
				$this->template->content->pagination_stats = '('.Kohana::lang('report.pagination.showing').' '
                     .$current_page.' '.Kohana::lang('report.pagination.of').' '.$total_pages
                     .' '.Kohana::lang('report.pagination.pages_of').' '.$pagination->total_items.' '.$report_string.')';

                $this->template->content->pagination = $pagination;
			} else { // If we don't want to show pagination
				$this->template->content->pagination_stats = '('.$pagination->total_items.' '.$report_string.')';
			}
		} else {
			$this->template->content->pagination_stats = '('.$pagination->total_items.' '.$report_string.')';
		}
